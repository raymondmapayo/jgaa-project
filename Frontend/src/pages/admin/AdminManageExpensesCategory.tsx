...
import AddExpensesCategoryModal from "../AdminModals/AddExpensesCategoryModal";
...
const AdminManageExpensesCategory = () => {
  const [dataSource, setDataSource] = useState<ExpensesCategoryItem[]>([]);
  ...
  const handleEdit = (record: ExpensesCategoryItem) => {
    setSelectedItem(record);
    setIsEditModalVisible(true);
  };

  const handleSaveEdit = (updatedCategory: ExpensesCategoryItem) => {
    setDataSource((prevData) =>
      prevData.map((cat) =>
        cat.expenses_category_id === updatedCategory.expenses_category_id
          ? updatedCategory
          : cat,
      ),
    );
  ...
  const handleDelete = async (expenses_category_id: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          await axios.delete(
            `${apiUrl}/categories_expenses_delete/${expenses_category_id}`,
          );
        } catch (error) {
        ...
        <div className="flex gap-2">
          ...
          <Tooltip title="Delete Category">
            <ActionButton
              danger
              icon={<DeleteOutlined />}
              onClick={() => handleDelete(record.expenses_category_id)}
            />
          </Tooltip>
        </div>
      ),
...
export default AdminManageExpensesCategory;
