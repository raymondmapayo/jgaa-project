...
const WorkerManageDrinksInventory = () => {
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      try {
        const response = await axios.get(`${apiUrl}/get_inventory_drinks`);
  ...
  const columns = [
    {
      title: "ID",
      dataIndex: "drinks_inventory_id",
      key: "drinks_inventory_id",
      render: (_text: any, record: any, _index: number) =>
        `${record.drinks_inventory_id.toString().padStart(3, "0")}`,
    },
  ...
  return (
    <StyledContainer>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div>
          <h2 className="text-lg font-bold">Drinks Inventory</h2>
          <p className="text-gray-500 text-sm">Manage your drinkss inventory</p>
        </div>
      </div>
...
export default WorkerManageDrinksInventory;
