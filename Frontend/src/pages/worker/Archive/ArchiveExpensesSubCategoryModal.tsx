...
const ArchiveExpensesSubCategoryModal = ({
  ...
  const handleRestore = async (expenses_subcategory_id: number) => {
    Modal.confirm({
      ...
      content: "This will restore the subcategory back to active status.",
      okText: "Yes, Restore",
      okType: "primary",
      cancelText: "Cancel",
      onOk: async () => {
      ...
      render: (_: any, record: UsedSupplyCategoryItem) => (
        <Tooltip title="Restore Category">
          <Button
            type="primary"
            icon={<UploadOutlined />}
            onClick={() => handleRestore(record.expenses_subcategory_id)}
          >
            Restore
          </Button>
        </Tooltip>
  ...
  return (
    <Modal
      title="Archived Expense Sub Categories"
    ...
    >
      {isLoading ? (
        ...
        <Table
          ...
          columns={columns}
          rowKey="expenses_subcategory_id"
...
export default ArchiveExpensesSubCategoryModal;
